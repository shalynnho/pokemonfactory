...
import factory.PartsRobotAgent.Arm;
import factory.data.Part;

public class v0_PartsRobotCameraNormativeScenario extends TestCase {
	...
	public void testNormativeScenario() throws InterruptedException {
		...
		assertEquals("Camera Agent should have 1 nest agent in its NestAgents List",1,camera.nests.size());
		...
		camera.pickAndExecuteAnAction();
		System.out.println("camera should send partsList to partrobot and remove nest from list of nests");
		assertEquals("Camera Agent should have 0 nest agent in its NestAgents List",0,camera.nests.size());
		assertEquals("PartsRobot should have a single nest in its GoodParts map",1,parts.GoodParts.size());
		parts.pickAndExecuteAnAction();
		assertEquals("Nest Agent should have a part removed by Parts Robot",8,nest.count);
		assertEquals("Nest Agent's state should change from TakingParts to Not TakingParts",false,nest.takingParts);
	}
}
