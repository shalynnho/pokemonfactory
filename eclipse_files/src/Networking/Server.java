...
/**
 * The Server is the "middleman" between Agents and the GUI clients. This is where constructors of the different Agents
 * will be called, as well as establishing connections with the GUI clients.
 * 
 * @author Peter Zhang
 */
public class Server {
	...
	devices.put(Constants.KIT_ROBOT_TARGET, new KitRobotGraphics(this, agents.get(Constants.KIT_ROBOT_TARGET),
		agents.get(Constants.STAND_TARGET)));
	devices.put(Constants.PARTS_ROBOT_TARGET,
		new PartsRobotGraphics(this, agents.get(Constants.PARTS_ROBOT_TARGET)));
	devices.put(Constants.STAND_TARGET + 0,
		new InspectionStandGraphics(this, agents.get(Constants.STAND_TARGET + 0)));

	for (int i = 0; i < Constants.FEEDER_COUNT; i++) {
	    devices.put(Constants.FEEDER_TARGET + i,
		    new FeederGraphics(i, this, agents.get(Constants.FEEDER_TARGET + i)));
	}
	for (int i = 0; i < Constants.LANE_COUNT; i++) {
	    devices.put(Constants.LANE_TARGET + i, new LaneGraphics(this, i, agents.get(Constants.LANE_TARGET + i)));
	}
	for (int i = 0; i < Constants.NEST_COUNT; i++) {
	    devices.put(Constants.NEST_TARGET + i, new NestGraphics(this, i, agents.get(Constants.NEST_TARGET + i)));
	}
	...
	for (int i = 0; i < Constants.FEEDER_COUNT; i++) {
	    ...
	    ((FeederAgent) agents.get(Constants.FEEDER_TARGET + i)).setGantry((GantryAgent) agents
		    .get(Constants.GANTRY_ROBOT_TARGET));
	    ((GantryAgent) agents.get(Constants.GANTRY_ROBOT_TARGET)).setFeeder((FeederAgent) agents
		    .get(Constants.FEEDER_TARGET + i));
	}

	for (int i = 0; i < Constants.LANE_COUNT; i++) {
	    ((LaneAgent) agents.get(Constants.LANE_TARGET + i)).setFeeder((FeederAgent) agents
		    .get(Constants.FEEDER_TARGET + i / 2));
	    ((LaneAgent) agents.get(Constants.LANE_TARGET + i)).setNest((NestAgent) agents.get(Constants.NEST_TARGET
		    + i));
	    ((NestAgent) agents.get(Constants.NEST_TARGET + i)).setLane((LaneAgent) agents.get(Constants.LANE_TARGET
		    + i));
	    ((NestAgent) agents.get(Constants.NEST_TARGET + i)).setCamera((CameraAgent) agents
		    .get(Constants.CAMERA_TARGET));
	    ((CameraAgent) agents.get(Constants.CAMERA_TARGET)).setNest((NestAgent) agents.get(Constants.NEST_TARGET
		    + i));
	...
	((StandAgent) agents.get(Constants.STAND_TARGET)).setPartsRobot(partsrobot);
	...
	// V0 heck
	/*
	 * NestAgent nest0 = (NestAgent) agents.get(Constants.NEST_TARGET + 0); NestAgent nest1 = (NestAgent)
	 * agents.get(Constants.NEST_TARGET + 1); //nest0.FillWithParts(); //nest1.FillWithParts(); // end V0 heck
	 * 
	...
	((FCSAgent) agents.get(Constants.FCS_TARGET))
		.setGantry((GantryAgent) agents.get(Constants.GANTRY_ROBOT_TARGET));
	((FCSAgent) agents.get(Constants.FCS_TARGET)).setPartsRobot((PartsRobotAgent) agents
