...
public class KitRobotGraphicsDisplay extends DeviceGraphicsDisplay {
	...
	public void moveToInitialOrFinal() {
		...
		if (position.equals(Position.conveyorPosition)) {
			if (moveToPosition.equals(Command.moveToInspectionStand)) {
				setVectorConfigurations(inspectionLocation.getX()
						- kitEnterLocation.getX(), inspectionLocation.getY()
						- kitEnterLocation.getY(), Position.inspectionPosition);
			} else if (moveToPosition.equals(Command.moveToLocation1)) {
				setVectorConfigurations(
						location1.getX() - kitEnterLocation.getX(),
						location1.getY() - kitEnterLocation.getY(),
						Position.location1Position);
			} else if (moveToPosition.equals(Command.moveToLocation2)) {
				setVectorConfigurations(
						location2.getX() - kitEnterLocation.getX(),
						location2.getY() - kitEnterLocation.getY(),
						Position.location2Position);
			} else if (moveToPosition.equals(Command.moveToGoodConveyor)) {
				setVectorConfigurations(kitLeaveLocation.getX()
						- kitEnterLocation.getX(), kitLeaveLocation.getY()
						- kitEnterLocation.getY(),
						Position.goodConveyorPosition);
			} else {
			...
			if (moveToPosition.equals(Command.moveToLocation1)) {
				setVectorConfigurations(
						location1.getX() - inspectionLocation.getX(),
						location1.getY() - inspectionLocation.getY(),
						Position.location1Position);
			} else if (moveToPosition.equals(Command.moveToLocation2)) {
				setVectorConfigurations(
						location2.getX() - inspectionLocation.getX(),
						location2.getY() - inspectionLocation.getY(),
						Position.location2Position);
			} else if (moveToPosition.equals(Command.moveToConveyor)) {
				setVectorConfigurations(kitEnterLocation.getX()
						- inspectionLocation.getX(), kitEnterLocation.getY()
						- inspectionLocation.getY(), Position.conveyorPosition);
			} else if (moveToPosition.equals(Command.moveToGoodConveyor)) {
				setVectorConfigurations(kitLeaveLocation.getX()
						- inspectionLocation.getX(), kitLeaveLocation.getY()
						- inspectionLocation.getY(),
						Position.goodConveyorPosition);
			} else {
			...
			if (moveToPosition.equals(Command.moveToLocation2)) {
				setVectorConfigurations(location2.getX() - location1.getX(),
						location2.getY() - location2.getY(),
						Position.location2Position);
			} else if (moveToPosition.equals(Command.moveToConveyor)) {
				setVectorConfigurations(
						kitEnterLocation.getX() - location1.getX(),
						kitEnterLocation.getY() - location1.getY(),
						Position.conveyorPosition);
			} else if (moveToPosition.equals(Command.moveToInspectionStand)) {
				setVectorConfigurations(
						inspectionLocation.getX() - location1.getX(),
						inspectionLocation.getY() - location1.getY(),
						Position.inspectionPosition);
			} else if (moveToPosition.equals(Command.moveToGoodConveyor)) {
				setVectorConfigurations(
						kitLeaveLocation.getX() - location1.getX(),
						kitLeaveLocation.getY() - location1.getY(),
						Position.goodConveyorPosition);
			} else {
			...
			if (moveToPosition.equals(Command.moveToConveyor)) {
				setVectorConfigurations(
						kitEnterLocation.getX() - location2.getX(),
						kitEnterLocation.getY() - location2.getY(),
						Position.conveyorPosition);
			} else if (moveToPosition.equals(Command.moveToInspectionStand)) {
				setVectorConfigurations(
						inspectionLocation.getX() - location2.getX(),
						inspectionLocation.getY() - location2.getY(),
						Position.inspectionPosition);
			} else if (moveToPosition.equals(Command.moveToLocation1)) {
				setVectorConfigurations(location1.getX() - location2.getX(),
			...
			if (moveToPosition.equals(Command.moveToConveyor)) {
				setVectorConfigurations(kitEnterLocation.getX()
						- kitLeaveLocation.getX(), kitEnterLocation.getY()
						- kitLeaveLocation.getY(), Position.conveyorPosition);
			} else if (moveToPosition.equals(Command.moveToInspectionStand)) {
				setVectorConfigurations(inspectionLocation.getX()
						- kitLeaveLocation.getX(), inspectionLocation.getY()
						- kitLeaveLocation.getY(), Position.inspectionPosition);
			} else if (moveToPosition.equals(Command.moveToLocation1)) {
				setVectorConfigurations(
						location1.getX() - kitLeaveLocation.getX(),
						location1.getY() - kitLeaveLocation.getY(),
						Position.location1Position);
			} else if (moveToPosition.equals(Command.moveToLocation2)) {
				setVectorConfigurations(
						location2.getX() - kitLeaveLocation.getX(),
						location2.getY() - kitLeaveLocation.getY(),
						Position.location2Position);
			} else {
	...
	public void sendDoneMessage() {
		if (this.sendMessage.equals(Message.sendStand1DoneMessage)) {
			...
			kitRobotClient.sendData(new Request(
					Constants.KIT_ROBOT_ON_STAND2_DONE,
					Constants.KIT_ROBOT_TARGET, currentKit.getKitConfig()));
		...
		if (target.equals(Constants.KIT_ROBOT_TARGET)) {
			if (command
				...
				InspectionToLocation1();
					...
					.equals(Constants.KIT_ROBOT_DISPLAY_PICKS_INSPECTION_TO_LOCATION2)) {
				KitGraphicsDisplay tempKit = new KitGraphicsDisplay(
						(KitConfig) obj);
				tempKit.setLocation(inspectionLocation);
				setKitConfigurations(tempKit, 2);
	...
	public void doJob() {
		if (!jobIsDone) {
			...
			stepCountDown -= 1;
			if (finalJob) {
				currentKit.setLocation(new Location(currentKit.getLocation()
						.getX() + this.xStep, currentKit.getLocation().getY()
						+ this.yStep));
			}
	...
	public void draw(JComponent c, Graphics2D g) {
		...
		drawtheKits(c, g);
		// g.drawImage(Constants.KIT_ROBOT_IMAGE_FLICKER, kitRobotPositionX,
		// kitRobotPositionY, c);
		animateRobot(g, c);
	}
	...
	public void animateRobot(Graphics2D g, JComponent c) {
		img = Toolkit.getDefaultToolkit().getImage(
				Constants.KIT_ROBOT_IMAGE + seq + ".png");
		if (animateKitRobot) {		
			if (animCount % 7 == 0) {
				sequenceIncrease();
			} else if (animCount < 1) {
