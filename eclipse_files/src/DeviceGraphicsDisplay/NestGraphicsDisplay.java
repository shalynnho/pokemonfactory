...
public class NestGraphicsDisplay extends DeviceGraphicsDisplay {
	...
	public NestGraphicsDisplay(Client c, int id) {
		...
		partsInNest = Collections
				.synchronizedList(new ArrayList<PartGraphicsDisplay>());
		partStartLoc = new Location(Constants.LANE_END_X
				- Constants.PART_PADDING, location.getY()
				+ Constants.PART_WIDTH / 2 - Constants.PART_OFFSET);
		purgeLoc = new Location(location.getX()
	...
	private void animatePurge() {
		...
		if (numRemaining > 0) {
			for (int i = 2; i < numRemaining + 2; i++) {
				updateXLoc(partsInNest.get(i).getLocation(),
						partLocs.get(i - 2), 3);
				updateYLoc(partsInNest.get(i).getLocation(),
						partLocs.get(i - 2), 1);
			}
		}
	}
		...
		for (int i = 0; i < MAX_PARTS; i++) {
			if (i % 2 == 0) { // top row
				partLocs.add(new Location(location.getX() + i / 2
						* Constants.PART_WIDTH - Constants.PART_PADDING_NEST,
						location.getY() - Constants.PART_OFFSET));
			} else { // bottom row
				partLocs.add(new Location(location.getX() + i / 2
						* Constants.PART_WIDTH - Constants.PART_PADDING_NEST,
						location.getY() + BOTTOM_ROW_OFFSET
	...
	private void givePartToPartsRobot() {
		// TODO: V2: are we supporting PR removal of parts from any loc in the
		// nest?
		partsInNest.remove(0);
		setPartLocations();
	...
	/**
	 * Tells the agent that the part has reached its place in the nest. Make
	 * sure only sends message once for each part, not on every call to draw.
	 */
	private void msgAgentReceivePartDone() {
		if (!receivePartDoneSent) {
			client.sendData(new Request(Constants.NEST_RECEIVE_PART_COMMAND
					+ Constants.DONE_SUFFIX, Constants.NEST_TARGET + nestID,
					null));
			// System.out.println("	NESTGD" + nestID +
